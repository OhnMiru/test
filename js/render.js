    ...
    currentFilteredData.forEach((card, displayIndex) => {
        ...
        cardDiv.innerHTML = `
            <div class="info">
                ...
                <div class="stock-row ${stockWarningClass}" style="margin-bottom: 2px;">
                    <span class="stock">Остаток: ${stock} шт</span>
                    ${isStockWarning ? '<span style="margin-left: 8px; font-size: 12px;">⚠️</span>' : ''}
                </div>
...
function updateCardInDOM(cardId, updatedData) {
    ...
    const stockRow = cardElement.querySelector('.stock-row');
        ...
        if (stockRow) {
            if (isStockWarning) {
                ...
                if (!stockRow.querySelector('.stock-warning-icon')) {
                    ...
                    stockRow.appendChild(warningIcon);
                }
            } else {
                stockRow.classList.remove('stock-warning');
                const warningIcon = stockRow.querySelector('.stock-warning-icon');
